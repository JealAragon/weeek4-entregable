...
const verifyUser =catchError(async(req,res)=>{
    ...
    if(!userCode) return  res.status(401).json({error: 'User no found'})
...
const resetPassword= catchError(async(req, res)=>{
    ...
    //buscamos al usuario 
    const user = await User.findOne({where:{email}})
    if(!user) return res.send(401).json({error:"User no found"})
...
const updatePassword = catchError(async(req,res)=>{
    ...
    if(!codeUser) return res.send(401).json({error:"User no found"})
